...
def init_db():
    ...
    try:
        conn.execute("ALTER TABLE users ADD COLUMN nom_entreprise TEXT DEFAULT ''")
        conn.commit()
    except Exception:
        pass
...
def api_register():
    ...
    conn = get_db()
    try:
        conn.execute("INSERT INTO users (nom, email, mot_de_passe, mode, nom_entreprise) VALUES (?, ?, ?, ?, ?)",
            (data["nom"], data["email"], hash_mdp(data["mot_de_passe"]), data.get("mode", "perso"), data.get("nom_entreprise", "")))
        conn.commit()
...
def modifier_profil():
    ...
    conn = get_db()
    try:
        conn.execute("UPDATE users SET nom=?, email=? WHERE id=?", (data["nom"], data["email"], session["user_id"]))
        conn.commit()
...
def changer_mdp():
    ...
    conn.execute("UPDATE users SET mot_de_passe=? WHERE id=?", (hash_mdp(data["nouveau"]), session["user_id"]))
    conn.commit()
    conn.close()
    return jsonify({"succes": True})
...
def liste_transactions():
    ...
    params = [uid]
    if type_filtre: query += " AND type=?"; params.append(type_filtre)
    if categorie: query += " AND categorie=?"; params.append(categorie)
    if date_debut: query += " AND date_ajout>=?"; params.append(date_debut)
    if date_fin: query += " AND date_ajout<=?"; params.append(date_fin)
...
def liste_salaires():
    ...
    conn = get_db()
    query = "SELECT s.*, e.nom as employe_nom, e.poste as employe_poste FROM salaires s JOIN employes e ON s.employe_id=e.id"
    params = []
    if mois: query += " WHERE s.mois=?"; params.append(mois)
...
def generer_salaires():
    ...
    for e in employes:
        conn.execute("INSERT INTO salaires (employe_id,mois,montant,bonus,deductions,net_paye) VALUES (?,?,?,0,0,?)",
            (e["id"], mois, e["salaire_base"], e["salaire_base"]))
    conn.commit(); conn.close()
...
def pdf_salaire(id):
    ...
    elements.append(Spacer(1, 20))
    data_table = [["Description", "Montant (FCFA)"],
